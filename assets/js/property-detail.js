...
const properties = {
  'hacienda-tenorio': {
    ...
    description: 'Propiedad de mayor extensión en Costa Rica con origen desde 1600. Conformada por 6 fincas agrupadas en 3 sociedades, libres de gravámenes. Alta accesibilidad con 17 km de frente a carretera hacia Upala y Tierra Morenas.',
  ...
  'finca-playa-buena-vista': {
    ...
    price: '$3,850,000',
    availability: 'Disponible',
    description: 'Dos fincas estratégicamente ubicadas en uno de los corredores turísticos más importantes del país. Propiedad principal de 75 ha con finca adicional en elevación. Combinación única de extensión, privacidad y cercanía a destinos de alta demanda.',
  ...
  'finca-tempate': {
    ...
    price: '$2,150,000',
    availability: 'Disponible',
    description: 'Ubicada a 5 km de Playa Potrero, en una de las zonas con mayor crecimiento y proyección turística de Costa Rica. Accesibilidad, entorno natural y conexión con los destinos clave del Pacífico Norte.',
...
function loadPropertyDetail() {
  ...
  availabilityEl.classList.add(property.availability === 'Disponible' ? 'available' : 'not-available');

  // Llenar descripción
  document.getElementById('detailDescription').textContent = property.description;

  // Llenar estadísticas
  const statsHTML = property.stats.map(stat => `
    <div class="stat">
      <span class="stat-value">${stat.value}</span>
      <span class="stat-label">${stat.label}</span>
    </div>
  `).join('');
  document.getElementById('detailStats').innerHTML = `<div style="display: flex; gap: 1.5rem; padding: 1rem 0; border-top: 1px solid rgba(201, 168, 76, 0.12); border-bottom: 1px solid rgba(201, 168, 76, 0.12);">${statsHTML}</div>`;
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (contactBtn) {
    contactBtn.addEventListener('click', function() {
      const propertyId = getPropertyIdFromUrl();
      const property = properties[propertyId];
      alert(`Gracias por tu interés en ${property.title}.\n\nNos pondremos en contacto pronto.`);
